...
class ResearchTask:
    ...
    def research(self, agent, task):
        return Task(description=dedent(f"""Conduct market research and competitor analysis on {task}
                                       ...
                                       Provide a earnings report in the competitor analysis
                                       Detailed and deep market study and competition analysis.
            **Note**: {self.__tip_section()}"""),
            expected_output="A detailed market research and competition analysis with their earnings report",
            agent=agent,)
    def generate_report(self, agent, task):
        ...
        INCLUDE ANNUAL REPORTS[URL AS USED IN A SEPERATE FOOTER SECTION] AND THEIR FINDINGS.                   
            500 words
    ...
    def case_research(self, agent, task):
        return Task(description=dedent(f"""UNDERSTAND THE TRENDS IN THE INDUSTRY OF {task}
                                       Research on atleast 20 ways to implement AI, Genai and ML in the industry to improve strategic focus areas (e.g., operations, supply chain, customer experience, etc.)
                                       ...
                                       Highlight atleast 20 cases where {task} can implement AI, genai and ML.
                                       """),
                expected_output=dedent(f"""A detailed research on implementation of AI, Genai and ML in industry of {task} and 20 use cases of implementation of AI for {task}"""),
                agent=agent,)
    def generate_case(self, agent, task):
        return Task(description=dedent(f"""FOR THE CONTEXT understand the industry trends and the implementation of AI.
                                       Generate a report for a cliet {task} detailing atleast 20 cases of implementation of gen-ai.
                                       Highlight atleast 20 cases where {task} can implement AI, genai and ML.
                                       GIVE A GOOD DESCRIPTION OF EACH CASE atleast 100 words 
    ...
    def db_generate(self, agent, task):
        return Task(description=dedent(f"""Identify databases/datasets MENTION URL for 10 cases of implementation of AI for {task}
                ...
                """),
        expected_output=dedent(f"""Atleast 10 databases/datasets for 10 cases for implementation of AI, ML, GenAI for {task} 
                               For each case provide technologies used, the challenges faced, and the solutions proposed."""),
        agent=agent,
        )